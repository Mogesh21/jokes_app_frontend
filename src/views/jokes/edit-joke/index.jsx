...
const EditJoke = () => {
  ...
  useEffect(() => {
    if (state.cat_id) {
      fetchCategoryById(state.cat_id);
      fetchSubcategoriesByCatId(state.cat_id);
      form.setFieldsValue({ cat_id: state.cat_id, subcat_id: state.subcat_id || '', content: state.content });
      if (state.content.joke_image) {
        const image = [
          {
            uid: '-1',
            name: 'Joke/Content Image',
            status: 'done',
            url: state.content.joke_image,
            thumbUrl: state.content.joke_image
          }
        ];
        form.setFieldValue('jokeImage', image);
        setJokeImage(image);
      }
      if (state.content.speaker_image) {
        const image = [
          {
            uid: '-1',
            name: 'Speaker Image',
            status: 'done',
            url: state.content.speaker_image,
            thumbUrl: state.content.speaker_image
          }
        ];
        form.setFieldValue('speakerImage', image);
        setSpeakerImage(image);
      }
      if (state.content.receiver_image) {
        const image = [
          {
            uid: '-1',
            name: 'Receiver Image',
            status: 'done',
            url: state.content.receiver_image,
            thumbUrl: state.content.receiver_image
          }
        ];
        form.setFieldValue('receiverImage', image);
        setReceiverImage(image);
      }
      if (state.content.image_answer) {
        const image = [
          {
            uid: '-1',
            name: 'Answer Image',
            status: 'done',
            url: state.content.image_answer,
            thumbUrl: state.content.image_answer
          }
        ];
        form.setFieldValue('imageAnswer', image);
        setImageAnswer(image);
      }
      if (state.content.conversation && state.content.conversation.length > 0) {
        setConversation(state.content.conversation);
      }
    } else {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (jokeImage && jokeImage.length > 0 && jokeImage[0].originFileObj) formData.append('joke_image', jokeImage[0].originFileObj);
      if (speakerImage && speakerImage.length > 0 && speakerImage[0].originFileObj)
        formData.append('speaker_image', speakerImage[0].originFileObj);
      if (receiverImage && receiverImage.length > 0 && receiverImage[0].originFileObj)
        formData.append('receiver_image', receiverImage[0].originFileObj);
      if (imageAnswer && imageAnswer.length > 0 && imageAnswer[0].originFileObj)
        formData.append('image_answer', imageAnswer[0].originFileObj);
      if (conversation && conversation.length > 1) values.content.conversation = conversation;
      if (state.content.joke_image) values.content.joke_image = state.content.joke_image;
      if (state.content.image_answer) values.content.image_answer = state.content.image_answer;
      if (state.content.speaker_image) values.content.speaker_image = state.content.speaker_image;
      if (state.content.receiver_image) values.content.receiver_image = state.content.receiver_image;
      formData.append('content', JSON.stringify(values.content));
      const response = await axiosInstance.put(`/jokes/${state.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      if (response.status === 200) {
        notification.success({ message: 'Joke updated successfully' });
        navigate('/app/dashboard/jokes/jokes-list');
      } else {
  ...
  return (
    <Layout className="layout" style={{ backgroundColor: 'white', borderRadius: '10px', padding: '1rem' }}>
      <p className="form-title">Update Joke</p>
